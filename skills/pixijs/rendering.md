...
The `container` property is the scene root to draw. `target` is a separate property that specifies a render destination (e.g., a [RenderTexture](rendering.RenderTexture.md)).
...
If your `target` is a [Texture](rendering.Texture.md) with a `frame` (e.g. an atlas sub-texture), that frame is interpreted in **mip 0** pixel space and is scaled/clamped when rendering to `mipLevel > 0`.
