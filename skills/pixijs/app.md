...
The [Application](app.Application.md) class is the starting point for most PixiJS projects. It sets up a renderer, creates a scene graph, and runs a frame loop so you can focus on building your game or visualization. It supports WebGL, WebGPU, and Canvas2D through async initialization.
...
By default, the [TickerPlugin](app.TickerPlugin.md) handles the rendering loop:
...
The [ResizePlugin](app.ResizePlugin.md) enables automatic resizing to fit different containers or the window:
...
- **Ticker Plugin** - Updates every frame. [Guide](app-1.md)
- **Resize Plugin** - Resizes renderer/canvas. [Guide](app-2.md)
- **Culler Plugin** (optional) - Culls offscreen objects. [Guide](app-3.md)
