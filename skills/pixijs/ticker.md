...
The [Ticker](ticker.Ticker.md) class executes callbacks on every animation frame. It handles game loops, animations, and any time-based updates with priority ordering and frame rate control.
...
PixiJS provides a global [Ticker](ticker.Ticker.md) instance at `Ticker.shared`. The [Application](app.Application.md) uses it by default, so most apps won't need to create their own.
