...
After loading, images still need to be decoded and uploaded to the GPU. For many images, this can cause a visible lag spike on first render. Use the [PrepareSystem](rendering.PrepareSystem.md) to pre-upload textures to the GPU before they appear on screen:
