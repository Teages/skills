...
`Assets.load()` returns a [GifSource](gif.GifSource.md), which holds all decoded frames. Pass it to [GifSprite](gif.GifSprite.md) to display and animate it.
...
> [!NOTE]
> GIFs are convenient but decode every frame into a separate texture. For performance-critical animations with many frames, consider using a spritesheet with [AnimatedSprite](scene.AnimatedSprite.md) instead; it uses a single atlas texture and is more GPU-friendly.

You can also pass an options object with the source and any [SpriteOptions](scene.SpriteOptions.md):
...
The asset loader accepts configuration through the `data` property. These options are forwarded to [GifSource.from](gif.GifSource.html#from) and extend `CanvasSourceOptions`.
...
All properties of [GifSpriteOptions](gif.GifSpriteOptions.md):
...
`clone()` creates a new [GifSprite](gif.GifSprite.md) sharing the same [GifSource](gif.GifSource.md). The clone copies playback settings but gets its own playback state.
...
- [GifSprite](gif.GifSprite.md) - animated sprite for GIF playback
- [GifSource](gif.GifSource.md) - decoded GIF frame data, created by the asset loader or `GifSource.from()`
- [GifSpriteOptions](gif.GifSpriteOptions.md) - constructor options for GifSprite
- [GifAsset](gif.GifAsset.md) - asset loader plugin (auto-registered on import)
- [GifFrame](gif.GifFrame.md) - per-frame data (texture, start/end timing in ms)
- [GifBufferOptions](gif.GifBufferOptions.md) - options for `GifSource.from()` and the asset loader's `data` field
